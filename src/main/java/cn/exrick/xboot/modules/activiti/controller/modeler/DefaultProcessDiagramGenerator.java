...
public class DefaultProcessDiagramGenerator implements ProcessDiagramGenerator {
    ...
    public DefaultProcessDiagramGenerator(final double scaleFactor) {
        ...
        this.artifactDrawInstructions.put(Association.class, new DefaultProcessDiagramGenerator.ArtifactDrawInstruction() {
            ...
            public void draw(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, Artifact artifact) {
                ...
                graphicInfoList = DefaultProcessDiagramGenerator.connectionPerfectionizer(processDiagramCanvas, bpmnModel, sourceElement, targetElement, graphicInfoList);
    ...
    protected static GraphicInfo getLineCenter(List<GraphicInfo> graphicInfoList) {
        ...
        for (p1 = 1; p1 < graphicInfoList.size(); ++p1) {
            GraphicInfo graphicInfo = graphicInfoList.get(p1);
            graphicInfo1 = graphicInfoList.get(p1 - 1);
            if (p1 == 1) {
            ...
            length += Math.sqrt(Math.pow((int) graphicInfo.getX() - (int) graphicInfo1.getX(), 2.0D) + Math.pow((int) graphicInfo.getY() - (int) graphicInfo1.getY(), 2.0D));
            lengths[p1] = length;
        }
        ...
        graphicInfo1 = graphicInfoList.get(p1);
    ...
    public InputStream generateDiagram(BpmnModel bpmnModel, String imageType, List<String> highLightedActivities, List<String> highLightedFlows) {
        return this.generateDiagram(bpmnModel, imageType, highLightedActivities, highLightedFlows, null, null, null, null, 1.0D);
    }
