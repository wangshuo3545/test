...
public class ActTaskController {
    ...
    @Operation(summary = "流程流转历史")
    public Result historicFlow(@Parameter(description ="流程实例id") @PathVariable String id) {
    ...
    @Operation(summary = "任务节点审批通过")
    public Result pass(@Parameter(description ="任务id") @RequestParam String id,
                       @Parameter(description ="流程实例id") @RequestParam String procInstId,
                       @Parameter(description ="下个节点审批人") @RequestParam(required = false) String[] assignees,
                       @Parameter(description ="优先级") @RequestParam(required = false) Integer priority,
                       @Parameter(description ="意见评论") @RequestParam(required = false) String comment,
                       ActMessageVo m) {
        ...
        if (tasks != null && tasks.size() > 0) {
                ...
                if (assignees == null || assignees.length < 1) {
                    ...
                    if (users == null || users.isEmpty()) {
                        ...
                        if (list == null || list.isEmpty()) {
                            ...
                            for (User user : users) {
                                ...
                                // 异步发消息
                                messageUtil.sendActMessage(user.getId(), ActivitiConstant.MESSAGE_ID.MESSAGE_TODO.getId(), actBusiness,
                    ...
                    if (list == null || list.isEmpty()) {
                        for (String assignee : assignees) {
                            ...
                            // 异步发消息
                            messageUtil.sendActMessage(assignee, ActivitiConstant.MESSAGE_ID.MESSAGE_TODO.getId(), actBusiness,
                                    ActivitiConstant.MESSAGE_TODO_CONTENT, m.getSendMessage(), m.getSendSms(), m.getSendEmail());
                        }
                    }
                }
            ...
            actBusinessService.update(actBusiness);
            // 异步发消息
            messageUtil.sendActMessage(actBusiness.getUserId(), ActivitiConstant.MESSAGE_ID.MESSAGE_PASS.getId(), actBusiness,
    ...
    public Result passAll(@Parameter(description ="任务ids") @RequestParam String[] ids,
                          @Parameter(description ="意见评论") @RequestParam(required = false) String comment,
                          ActMessageVo m) {
    ...
    @Operation(summary = "任务节点审批驳回至指定历史节点")
    public Result backToTask(@Parameter(description ="任务id") @RequestParam String id,
                             @Parameter(description ="驳回指定节点key") @RequestParam String backTaskKey,
                             @Parameter(description ="流程实例id") @RequestParam String procInstId,
                             @Parameter(description ="流程定义id") @RequestParam String procDefId,
                             @Parameter(description ="原节点审批人") @RequestParam(required = false) String[] assignees,
                             @Parameter(description ="优先级") @RequestParam(required = false) Integer priority,
                             @Parameter(description ="意见评论") @RequestParam(required = false) String comment,
                             ActMessageVo m) {
    ...
    @Operation(summary = "任务节点审批驳回至发起人")
    public Result back(@Parameter(description ="任务id") @RequestParam String id,
                       @Parameter(description ="流程实例id") @RequestParam String procInstId,
                       @Parameter(description ="意见评论") @RequestParam(required = false) String comment,
                       ActMessageVo m) {
        ...
        actBusinessService.update(actBusiness);
        // 异步发消息
        messageUtil.sendActMessage(actBusiness.getUserId(), ActivitiConstant.MESSAGE_ID.MESSAGE_BACK.getId(), actBusiness,
    ...
    @Operation(summary = "批量驳回至发起人")
    public Result backAll(@Parameter(description ="流程实例ids") @RequestParam String[] procInstIds,
                          @Parameter(description ="意见评论") @RequestParam(required = false) String comment,
                          ActMessageVo m) {
    ...
    @Operation(summary = "委托他人代办")
    public Result delegate(@Parameter(description ="任务id") @RequestParam String id,
                           @Parameter(description ="委托用户id") @RequestParam String userId,
                           @Parameter(description ="流程实例id") @RequestParam String procInstId,
                           @Parameter(description ="意见评论") @RequestParam(required = false) String comment,
                           ActMessageVo m) {
        ...
        ProcessInstance pi = runtimeService.createProcessInstanceQuery().processInstanceId(procInstId).singleResult();
        ActBusiness actBusiness = actBusinessService.get(pi.getBusinessKey());
        messageUtil.sendActMessage(userId, ActivitiConstant.MESSAGE_ID.MESSAGE_DELEGATE.getId(), actBusiness,
    ...
    @Operation(summary = "删除任务历史")
    public Result deleteHistoric(@Parameter(description ="任务id") @RequestParam String[] ids) {
