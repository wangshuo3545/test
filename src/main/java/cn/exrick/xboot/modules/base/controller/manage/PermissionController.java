...
public class PermissionController {
    ...
    public Result<Permission> add(Permission permission) {
        ...
        // name唯一去重
        if (StrUtil.isNotBlank(permission.getName()) && CollectionUtil.isNotEmpty(permissionService.findByName(permission.getName()))) {
            return ResultUtil.error("英文名name已存在");
        }
        // 判断拦截请求的操作权限按钮名是否已存在
    ...
    public Result<Permission> edit(Permission permission) {
        ...
        // 若修改name 唯一去重
        if (!old.getName().equals(permission.getName()) && CollectionUtil.isNotEmpty(permissionService.findByName(permission.getName()))) {
            return ResultUtil.error("英文名name已存在");
        }
